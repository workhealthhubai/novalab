...
- **Background work through BullMQ** with four queues (`notifications`, `reports`, `documents`, `scheduled-jobs`). Workers run inside the API process for now and can be split into a worker deployment by importing only `QueueModule`. The scheduler reconciles PACS every two minutes, retries failed MWL operations with bounded exponential backoff and produces idempotent examination-due notification jobs at 30/7/1/0/-7 day thresholds.
...
- Radiology workflow (`apps/api/src/modules/radiology`): create a request → publish its server-generated accession number to DICOM Modality Worklist → the registered modality sends the acquired study to Orthanc with C-STORE → `POST /api/radiology/pacs/reconcile` matches stable studies by accession number + internal PatientID → `POST /api/radiology/:id/viewer-session` issues the viewer cookie and returns the OHIF URL → `GET /api/radiology/:id/preview` proxies a PNG preview. Site/device setup is documented in `infrastructure/orthanc/DEVICE-INTEGRATION.md`.
...
- `POST /api/radiology/:id/viewer-session` issues a separate 5-minute **viewer cookie** (`osgb_dicomweb`, httpOnly, path `/dicom-web`) that Nginx verifies through `GET /api/auth/verify-dicomweb` before proxying DICOMweb.
...
- **Radyoloji (Doktor Modülü → Radyoloji):** each request gets a DICOM accession number and is published to the station-scoped MWL. The incoming panel lists stable studies matching this tenant's outstanding accession + internal PatientID and opens the existing request; `POST /radiology/pacs/reconcile` performs deterministic linking. A two-minute background sweep performs the same reconciliation and retries failed MWL publish/cleanup operations up to five times with exponential backoff. The page shows tenant-scoped PACS connectivity and operational counts; failed orders can also be retried manually. PACS study tags, preview, study-scoped OHIF access, reporting and cancellation remain available. Unknown devices cannot C-STORE; registered AE Title and source IP checks are enabled. `pnpm dicom:test` runs the C-ECHO → MWL C-FIND → C-STORE acceptance path when DCMTK is installed. For local development without Nginx the Vite dev server mirrors the viewer routes (`/viewer` → OHIF, `/dicom-web` → Orthanc with `ORTHANC_BASIC_AUTH_B64` from the repo `.env`; the cookie check is Nginx/production only).
...
- E-mail/SMS providers in the notifications processor; virus scanning/OCR for uploaded documents. The examination-due sweep creates persistent in-app notifications, document workers verify the stored checksum and create image thumbnails, and the employee-report job renders a real PDF; external delivery providers and malware scanning remain.
- Physician-role enforcement / e-signature on examination approval. Appointment relations and patient/examination/radiology time-overlap checks are enforced server-side.
...
- Vendor-specific ECG, spirometry and audiometry adapters after each real device's supported interface and conformance statement are obtained.
